# Weight Entry Discharge Direction — Spec

## Purpose

Defines the `IsDischarge` flag on `WeightEntry` for provider-delivery pedido conversions — vehicles arriving already loaded and discharging into the almacén — and how it inverts `BruteWeight` arithmetic and ticket labeling relative to the normal (arrive-empty, load-up) case.
