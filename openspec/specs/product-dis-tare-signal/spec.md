# Product Dis-Tare Signal — Spec

## Purpose

Defines the purely informational `RequiresDisTaring` flag on `PedidoLine`, and clarifies that it never propagates to `WeightDetail` or drives any weighing-screen behavior — actual dis-tare capture remains the pre-existing, unrelated `WeightDetail.SecondaryTare` mechanism.
