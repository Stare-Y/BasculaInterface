# Weight Concurrency — Spec

## Purpose

Defines how the system prevents lost updates when multiple terminals concurrently modify the same `WeightEntry`. Uses PostgreSQL `xmin` as an optimistic concurrency token server-side and a transparent retry mechanism client-side.

## Requirements

### Requirement: WeightEntry uses optimistic concurrency via xmin
The system SHALL configure `WeightEntry` in EF Core to use the PostgreSQL `xmin` system column as a concurrency token (shadow property with `HasColumnType("xid")`, `ValueGeneratedOnAddOrUpdate()`, `IsConcurrencyToken()`). When two concurrent writes target the same `WeightEntry` row, the second write SHALL receive a `DbUpdateConcurrencyException`, which the service layer SHALL translate to a `WeightConcurrencyException` that the controller maps to an HTTP `409 Conflict` response.
