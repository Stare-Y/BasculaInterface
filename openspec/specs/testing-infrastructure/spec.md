...
Defines the project's automated test setup: a three-layer strategy (isolated unit tests, container-backed API-integration tests, a Windows-VM FlaUI bot harness), an xUnit category convention that keeps `dotnet test` green on a plain checkout and in CI, and the `.slnf` + GitHub Actions workflow that build and test the Linux-buildable projects on every push and PR. The dev environment is Linux; the MAUI client only runs on Windows, so no single layer covers everything.
