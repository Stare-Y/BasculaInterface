# Pedido Partial Fulfillment — Spec

## Purpose

Defines how a `PedidoLine` can be converted to weight across multiple deliveries instead of only once, with received/pending amounts computed on the fly rather than stored, and both line- and header-level conclusion derived from that computation.
